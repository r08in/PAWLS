## This function is to combine linear model from each dataset To form a group linear model
...
## This function is to combine multiple dataset each dataset is already in a group linear model
...
CombineDataset = function(XX, xinfo, pinfo, YY) {
    ...
    for (j in 1:p) {
        ...
        for (i in 1:m) {
            # for combine each xj in dataset i
            tempx = cbind(matrix(0, xinfo[i], leftColNum), XX[rowStart:(rowStart + xinfo[i] - 1), 
                colStart:(colStart + pinfo[j] - 1)], matrix(0, xinfo[i], colSum - leftColNum - pinfo[j]))
            Xj = rbind(Xj, tempx)
