...
paramPlot=function(res,param="both",label1=NULL,label2=NULL,xlimw=NULL,xlimb=NULL,ylimw=NULL,ylimb=NULL)
{
  logw=NULL
  loglambdaw=NULL 
  b=NULL
  ...
  {
    #par(mfrow=c(1,2))
    plot(loglambdab,b,xlim=xlimb,ylim=ylimb,xlab=expression(paste("-log(",lambda["1n"],")")),ylab=expression(hat(beta)[j]),pch="-")
    #identify(loglambdab,b,labels=label2)
    abline(v=-log(res$lambda2),lty=2,col="gray60")
    plot(loglambdaw,logw,xlim=xlimw,ylim=ylimw,xlab=expression(paste("-log(",lambda["2n"],")")),ylab=expression(-log(hat(w)[i])),pch="-")
