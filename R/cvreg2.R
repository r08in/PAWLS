cvreg2=function(x, y,lambda1=NULL,nlambda1=30,lambda2=NULL,nlambda2=30,beta0,w0,K=5,matlab=NULL,dout=FALSE)
{
  ...
  while (i<=L) 
  {
    ...
    lambda1=logSeq(maxLambda1,minLambda1,L1)
    ...
    for(k in 1:K)
    {
      ...
      for(l2 in 1:L2)
      {
        xx=cbind( c(rep(1,length(trainy)),rep(0,p)), rbind(trainx,diag(sqrt(lambda2[l2]),p)))
...
#         bs[l2,]=solve(H)%*%t(trainx)%*%trainy
    ...
    # check convergence
    shift=c(beta0-beta,w0-ifelse(w==1,0.99,w))
  ...
  list(beta=beta,w=w,index1=index1,index2=index2,lambda1=lambda1,lambda2=lambda2,iter=i)
